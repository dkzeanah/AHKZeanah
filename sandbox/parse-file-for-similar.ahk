...
FileRead, OutputVar, % A_ScriptDir "/output-data-sorted.txt"
array := A.split(OutputVar, "/\s{10,100}\w{3}, \w{3}\s\d+,\s\d{4}\sat\s\d*\:\d*\s\w{2}\s/")
filteredArray := fn_removeSimilar(array, 1.0)
output := A.join(filteredArray, "`n")
FileAppend, %output%, % A_ScriptDir "/output-data-sorted-filtered.txt"
msgbox, % "done: " filteredArray.count() " vs " array.count()
/*

FileRead, OutputVar, output-data.txt

Sort, OutputVar
sleep, 300
FileAppend, %OutputVar%,%A_SCRIPTDIR%\output-data-sorted.txt
*/
return
